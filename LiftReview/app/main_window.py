...
class MainWindow(QMainWindow):
    ...
    def _setup_menu(self):
        ...
        # Pose appearance submenu
        pose_menu = view_menu.addMenu("Skeleton &Appearance")

        increase_width = QAction("Increase Line Thickness", self)
        increase_width.triggered.connect(lambda: self._adjust_pose_thickness(1))
        pose_menu.addAction(increase_width)

        decrease_width = QAction("Decrease Line Thickness", self)
        decrease_width.triggered.connect(lambda: self._adjust_pose_thickness(-1))
        pose_menu.addAction(decrease_width)

        toggle_joints = QAction("Toggle Joint Dots", self)
        toggle_joints.setCheckable(True)
        toggle_joints.setChecked(True)
        toggle_joints.triggered.connect(self._toggle_pose_joints)
        pose_menu.addAction(toggle_joints)

        toggle_outline = QAction("Toggle Joint Outline", self)
        toggle_outline.setCheckable(True)
        toggle_outline.setChecked(True)
        toggle_outline.triggered.connect(self._toggle_pose_outline)
        pose_menu.addAction(toggle_outline)

        self._bar_path_action = QAction("Show &Bar Path", self)
        self._bar_path_action.setCheckable(True)
        self._bar_path_action.setChecked(False)
        self._bar_path_action.triggered.connect(self._toggle_bar_path)
        view_menu.addAction(self._bar_path_action)
    ...
    def _display_frame(self, frame: np.ndarray):
        ...
        # Phase 4: Render velocity tracker marker and path
        if self._velocity_tracker and self._velocity_tracker.is_tracking:
            ...
            if self._show_bar_path:
                display = self._velocity_tracker.render_path(display, frame_idx)
    ...
    def _adjust_pose_thickness(self, delta: int):
        """Increase or decrease skeleton line/joint thickness."""
        new_thickness = max(1, self._pose_renderer.line_thickness + delta)
        new_radius = max(1, self._pose_renderer.joint_radius + delta)
        self._pose_renderer.line_thickness = new_thickness
        self._pose_renderer.joint_radius = new_radius
        if self._current_frame is not None and self._pose_enabled:
            self._run_pose_if_enabled(self._current_frame)
            self._display_frame(self._current_frame)

    def _toggle_pose_joints(self, enabled: bool):
        self._pose_renderer.show_joints = enabled
        if self._current_frame is not None and self._pose_enabled:
            self._display_frame(self._current_frame)

    def _toggle_pose_outline(self, enabled: bool):
        self._pose_renderer.show_outline = enabled
        if self._current_frame is not None and self._pose_enabled:
            self._display_frame(self._current_frame)

    def _toggle_bar_path(self, enabled: bool):
        """Toggle drawing of the tracked bar path."""
        self._show_bar_path = enabled
        if self._current_frame is not None:
            self._display_frame(self._current_frame)
